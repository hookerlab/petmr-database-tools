""" doseInfoWithPET.py
    Usage:
        doseInfoWithPET.py
        doseInfoWithPET.py [<Directory_Path>]
    Options:
        Directory_Path      : Path to Dose_info.xls and PET files
"""
...
__author__ = 'jphan'
'''
Finds Dose_info.xls and corresponding PET files
Manipulates 'Dose_info.xls' data to required format
Outputs:
-JSON file of data
'''
#Gets starting directory to find dose_info.xls and PET files
pathname = ''
if __name__ == "__main__":
    args = docopt(__doc__)
    boo = False
    if args["<Directory_Path>"]:
        pathname = str(args["<Directory_Path>"])
    ...
    for dirName, subdirList, fileList in os.walk(pathname):
        ...
        for fname in fileList:
            if 'Dose_info.xls' in fname:
                ...
                framePath = os.path.join(dirName, fname)
                pathListPETset.append(framePath)
                with open(framePath, 'rb') as file_to_check:
                    data = file_to_check.read()
                    md5_returned = hashlib.md5(data).hexdigest()
                md5sumListSet.append(md5_returned)
        if len(pathListPETset) > 0:
            pathListPET.append(pathListPETset)
            md5sumList.append(md5sumListSet)
            pathListPETset = []
            md5sumListSet = []
